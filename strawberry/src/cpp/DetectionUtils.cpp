    ...
    const cv::Size &model_size)
{
    ...
    // 1. Letterbox 전처리
    const vector<string> &allowed_classes = {
    ...
    for (int idx : indices)
    {
        ...
        int obj_center_x = boxes[idx].x + boxes[idx].width / 2;
        int feature = (obj_center_x > frame_center_x) ? 1 : 0; // 오른쪽=1, 왼쪽=0
        detections.push_back({class_ids[idx], confidences[idx], boxes[idx], feature});
    }
